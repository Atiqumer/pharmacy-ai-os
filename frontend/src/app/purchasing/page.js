...
export default function PurchasingPage() {
  ...
  return (
    ...
    <main className="purchasing-workspace app-content min-h-screen px-4 py-6 md:px-7 md:py-7">
      <div className="mx-auto max-w-[1440px] space-y-7">
        <header className="flex flex-col justify-between gap-4 border-b border-slate-200 pb-6 sm:flex-row">
          ...
          <div className="flex flex-col items-start gap-1.5 sm:items-end"><div className="flex flex-wrap gap-2"><button onClick={() => setShowSupplierForm(true)} className="ui-secondary">Add supplier</button><button title={!suppliers.length || !products.length ? 'Add at least one supplier and one inventory product first' : undefined} onClick={() => { setEditingOrder(null); setOrderLines([{ product_id: '', quantity: 1, cost_price: '' }]); setShowOrderForm(true); }} disabled={!suppliers.length || !products.length} className="ui-primary">New purchase order</button></div>{(!suppliers.length || !products.length) && <p className="max-w-xs text-xs text-slate-500">Add a supplier and an inventory product to create an order.</p>}</div>
        </header>
      ...
      {showSupplierForm && <Modal title="Add supplier" onClose={() => setShowSupplierForm(false)}><form onSubmit={createSupplier} className="space-y-4"><label className="block text-sm text-slate-400">Supplier name<input required name="name" className="form-input" /></label><label className="block text-sm text-slate-400">Contact name<input name="contact_name" className="form-input" /></label><div className="grid gap-3 sm:grid-cols-2"><label className="text-sm text-slate-400">Phone<input name="phone" className="form-input" /></label><label className="text-sm text-slate-400">Email<input type="email" name="email" className="form-input" /></label></div><label className="block text-sm text-slate-400">Address<textarea name="address" className="form-input" /></label><SubmitButtons loading={actionLoading} onCancel={() => setShowSupplierForm(false)} label="Create supplier" /></form></Modal>}
...
function Modal({ title, children, onClose, wide = false }) {
  return <div className="ui-modal-backdrop fixed inset-0 z-50 grid place-items-center p-3 sm:p-4"><div role="dialog" aria-modal="true" aria-label={title} className={`ui-modal-panel w-full ${wide ? 'max-w-4xl' : 'max-w-lg'} max-h-[92vh] overflow-y-auto rounded-xl p-5 sm:p-6`}><div className="mb-5 flex items-center justify-between gap-4"><h2 className="text-xl font-semibold">{title}</h2><button onClick={onClose} className="ui-link-button">Close</button></div>{children}</div></div>;
}

function SubmitButtons({ loading, onCancel, label }) {
  return <div className="flex flex-wrap justify-end gap-2"><button type="button" onClick={onCancel} className="ui-secondary">Cancel</button><button disabled={loading} className="ui-primary">{loading ? 'Saving…' : label}</button></div>;
}
